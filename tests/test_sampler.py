...
def _():
    ...
    L = uniform(0, 10)
    tol = L * 1e-2
    uniform_sampler = MCMCSampler(lambda x: cst(x, 4), (mean - L, mean + L), 2**8)
    y = uniform_sampler.sample(2**8).flatten()  # Ok, in fact 2 ** 16 is nothing ...
    ...
    # Normal distribution
    var = L
    normal_sampler = MCMCSampler(lambda x: pdf_normal(x, mean, var), (mean - 5 * L, mean + 5 * L), 2**8)
    y = normal_sampler.sample(2**8).flatten()
    assert_allclose(np.mean(y), mean, rtol=tol, atol=tol)
